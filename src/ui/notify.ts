...
export function smartNotify(ctx: Ctx, n: Notice): boolean {
  ...
  if (a.kind === 'proposal') {
    ...
    title = '⚖️ Decision needed';
    text = text.replace(/^Decision needed:\s*/, '');
    buttons.push({ label: 'Decide', primary: true, run: () => { close(); openDecisions(ctx); } });
  } else return false;
  card.append(
    h('div', { class: 'nc-title' }, h('span', { text: title }), h('button', { class: 'nc-x', aria: { label: 'Dismiss' }, on: { click: close } }, '×')),
  ...
  while (stack.children.length > (window.innerWidth <= 900 ? 2 : 3)) stack.firstElementChild?.remove();
