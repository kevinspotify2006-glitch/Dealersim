...
import { buyLand, canPlace, findSpot, unlockReason, carDropCheck, carSize, duplicateSpot, footprint, lineCheck, lotStats, moveObject, nextPlot, paintZone, placeLine, placeObject, placeVehicle, zoneAt, zoneCost } from '../../sim/lot';
...
import type { BuildHost } from '../world/build';
import type { BuildContext, BuildView, Filter, SheetSize, Tool } from '../world/build';
import { contextAt, roomChooser, roomMenu } from '../world/buildroom';
import { BUILD_ROOM_BY_ID, allowedRooms, placementOf } from '../../data/buildrooms';
...
import { effectLines, objectInfo, vehicleInfo } from '../world/info';
import type { ObjDef } from '../../data/lot';
import { isMobileLandscape, isPhone, setCompact } from '../layout';
...
interface Ui {
  ...
  sheet: SheetSize;
  /** Build Mode 2.0: the drawer's view and the room being built in. */
  view: BuildView;
  context: BuildContext | null;
}

// Kept between visits so the dealership looks the way you left it.
const ui: Ui = { build: false, tool: { kind: 'select' }, category: 'zones', search: '', filter: 'all', styleTarget: 'all', grid: true, snap: true, panel: null, selected: null, moveCar: null, flow: false, cams: {}, sheet: 'half', view: 'rooms', context: null };
...
export function worldView(ctx: Ctx): View {
  ...
  // Desktop Build Mode 2.0: the selected item's details on the right while placing.
  const detailPanel = h('aside', { class: 'world-detail', aria: { label: 'Selected item' } });
  ...
  const root = h('div', { class: 'view world' }, canvas, hud, buildStrip, zoomBtns, banner, pop, paletteHost, buildBar, confirmBar, detailPanel, actionBar, panelHost);
  ...
  const frame = (now: number): void => {
    ...
    if (glide) {
      const k = Math.min(1, (performance.now() - glide.start) / 380);
      const e = 1 - (1 - k) ** 3;
      cam.x = glide.from.x + (glide.to.x - glide.from.x) * e;
      cam.y = glide.from.y + (glide.to.y - glide.from.y) * e;
      cam.zoom = glide.from.zoom + (glide.to.zoom - glide.from.zoom) * e;
      if (k >= 1) { glide = null; saveCam(); }
    }
    renderScene(canvas, {
      ...
      focus: ui.build && ui.view === 'room' && ui.context?.rect ? ui.context.rect : null,
  ...
  const wallSnap = (defId: string, x: number, y: number, rot: 0 | 1, ignore?: string): { x: number; y: number } => {
    const def = OBJ_BY_ID[defId];
    const wallItem = placementOf(def) === 'wall';
    if (!snapOn() || def.slot || def.line || def.edge || (Math.min(def.w, def.h) > 1 && !wallItem) || !ZONE_BY_CODE[zoneAt(loc.lot, x, y)]?.indoor) return { x, y };
    ...
    // Wall-mounted items (TVs, posters, logos…) look up to three metres for a wall.
    for (let reach = 1; reach <= (wallItem ? 3 : 1); reach += 1) {
      for (const [dx, dy] of [[0, -1], [0, 1], [-1, 0], [1, 0]]) {
        if (touches(x + dx * reach, y + dy * reach) && canPlace(ctx.state, loc, defId, x + dx * reach, y + dy * reach, rot, ignore, true).ok) return { x: x + dx * reach, y: y + dy * reach };
      }
    }
    return { x, y };
  };
  ...
  const commitPlace = (): void => {
    ...
    if (t.kind === 'place') {
      ...
      feedback(r);
      if (r.ok) placedFx(def.name, gh.x + (gh.rot ? def.h : def.w) / 2, gh.y + (gh.rot ? def.w : def.h) / 2, def.cost);
      ...
      if (isPhone() && next.kind === 'place') {
        const spot = ui.view === 'room' ? findSpotInRoom(t.defId) : findSpot(ctx.state, loc, t.defId);
  ...
  const placedFx = (name: string, x: number, y: number, cost = 0): void => {
    floaters.push({ text: `✓ ${name}`, x, y, born: time, tone: '#2fd18b' });
    if (cost) floaters.push({ text: `−${money(cost)}`, x, y: y + 0.9, born: time + 0.15, tone: '#ff9a4d' });
  };
  ...
  const onTap = (w: { x: number; y: number }, type: string): void => {
    ...
    if (ui.build) {
      ...
      if (obj) select({ kind: 'object', id: obj }, w);
        ...
        ui.selected = null;
        closePop();
        // Tapping a floor picks the room you build in: its menu opens.
        if (ui.view !== 'catalog' && w.x >= 0 && w.y >= 0 && w.x < loc.lot.w && w.y < loc.lot.h) {
          const tx = Math.floor(w.x);
          const ty = Math.floor(w.y);
          setBuildContext(contextAt(loc, tx, ty, zoneAt(loc.lot, tx, ty)), false);
        }
      }
      return;
    }
  ...
  const actions: WorldActions = {
    ...
    buildAt: (x, y) => {
      ...
      setBuildContext(contextAt(loc, tx, ty, zoneAt(loc.lot, tx, ty)));
    },
  ...
  const closePop = (): void => {
    ...
    popKey = '';
    pop.replaceChildren();
    pop.classList.remove('open', 'sheet', 'side');
    releasePop?.();
    releasePop = null;
  };
  const paintPop = (): void => {
    ...
    // Landscape phones: the card is a context panel on the right, not a sheet.
    const side = isMobileLandscape();
    const sheet = isPhone() && !side;
    ...
    if (side) pop.style.transform = '';
    else if (!sheet) placePop();
  };
  const placePop = (): void => {
    if (!popAnchor) return;
  ...
  const enterBuild = (category?: string, refit = true): void => {
    ...
    if (category) { ui.category = category; ui.view = 'catalog'; ui.context = null; }
    setGameMode('build');
    closePanel();
    closePop();
    stopMoveCar();
  ...
  let lastViewKey = '';
  let glide: { from: { x: number; y: number; zoom: number }; to: { x: number; y: number; zoom: number }; start: number } | null = null;
  /** A short, smooth camera move so a chosen room sits in the free part of the screen. */
  const glideToRect = (r: { x0: number; y0: number; x1: number; y1: number }): void => {
    const phone = isPhone();
    const side = phone && window.matchMedia('(max-height: 560px) and (orientation: landscape)').matches;
    const t = new Camera();
    t.resize(cam.width, cam.height);
    t.bounds = { ...cam.bounds };
    t.min = cam.min;
    t.max = Math.min(cam.max, 46);
    t.fit(r.x0 - 1.5, r.y0 - 1.5, r.x1 + 2.5, r.y1 + 2.5, {
      top: phone ? 76 : 70,
      bottom: phone && !side ? Math.round(Math.max(paletteHost.offsetHeight, cam.height * 0.36) + 12) : 96,
      left: (!phone || side) ? (paletteHost.offsetWidth || 340) + 16 : 16,
      right: detailPanel.classList.contains('open') ? (phone ? 270 : 320) : !phone ? 24 : 16,
    });
    if (document.documentElement.classList.contains('reduced-motion')) { cam.x = t.x; cam.y = t.y; cam.zoom = t.zoom; saveCam(); return; }
    glide = { from: { x: cam.x, y: cam.y, zoom: cam.zoom }, to: { x: t.x, y: t.y, zoom: t.zoom }, start: performance.now() };
  };
  /** Choose the room to build in (from the drawer or by tapping its floor). */
  const setBuildContext = (c: BuildContext | null, fromMenu = true): void => {
    const before = ui.context;
    ui.context = c;
    ui.view = c ? 'room' : 'rooms';
    const k = ui.tool.kind;
    const roomChanged = !before || !c || before.room !== c.room || before.rect?.x0 !== c.rect?.x0 || before.rect?.y0 !== c.rect?.y0;
    if ((k === 'place' || k === 'paint' || k === 'stamp' || k === 'room') && (roomChanged || before?.category !== c?.category)) ui.tool = { kind: 'select' };
    if (isPhone() && ui.sheet === 'peek') ui.sheet = 'half';
    paintBuild();
    if (c && roomChanged) {
      // A room glides into view; outdoor shows the whole lot.
      const r = c.rect ?? (c.room === 'outdoor' ? { x0: 0, y0: 0, x1: loc.lot.w - 1, y1: loc.lot.h - 1 } : null);
      if (r) requestAnimationFrame(() => glideToRect(r));
    }
    if (!fromMenu && c) haptic(8);
  };
  /** A free spot for an item inside the chosen room (nearest the middle; walls first for wall items). */
  const findSpotInRoom = (defId: string): { x: number; y: number; rot: 0 | 1 } | null => {
    const c = ui.context;
    if (!c?.rect) { const sp = findSpot(ctx.state, loc, defId); return sp ? { x: sp.x, y: sp.y, rot: sp.rot } : null; }
    const def = OBJ_BY_ID[defId];
    const r = c.rect;
    const cx = (r.x0 + r.x1) / 2;
    const cy = (r.y0 + r.y1) / 2;
    const wall = placementOf(def) === 'wall';
    let best: { x: number; y: number; rot: 0 | 1; d: number } | null = null;
    for (const rot of [0, 1] as const) {
      const w = rot ? def.h : def.w;
      const hh = rot ? def.w : def.h;
      for (let y = r.y0; y + hh - 1 <= r.y1; y += 1) for (let x = r.x0; x + w - 1 <= r.x1; x += 1) {
        if (!canPlace(ctx.state, loc, defId, x, y, rot, undefined, true).ok) continue;
        let d = Math.hypot(x + w / 2 - cx, y + hh / 2 - cy) + rot * 0.5;
        if (wall && x !== r.x0 && y !== r.y0 && x + w - 1 !== r.x1 && y + hh - 1 !== r.y1) d += 100;
        if (!best || d < best.d) best = { x, y, rot, d };
      }
    }
    if (best) return best;
    return { x: Math.round(cx - def.w / 2), y: Math.round(cy - def.h / 2), rot: 0 };
  };
  const paintBuild = (): void => {
    ...
    const trayLeft = paletteHost.querySelector('.br-tray')?.scrollLeft ?? 0;
    const catsLeft = paletteHost.querySelector('.br-cats')?.scrollLeft ?? 0;
    const viewKey = `${ui.view}|${ui.context?.room ?? ''}|${ui.context?.category ?? ''}`;
    const sameView = viewKey === lastViewKey;
    lastViewKey = viewKey;
    paletteHost.dataset.view = ui.view;
    const host: BuildHost = {
      ...
      setTool: (t) => {
        ...
        if (isPhone() && t.kind === 'place' && !OBJ_BY_ID[t.defId]?.line && t.x === undefined) {
          const spot = ui.view === 'room' ? findSpotInRoom(t.defId) : findSpot(ctx.state, loc, t.defId);
          if (spot) { t.x = spot.x; t.y = spot.y; t.rot = spot.rot; if (ui.view !== 'room' || !ui.context?.rect) centerOn(spot.x + 0.5, spot.y + 0.5); }
        }
        paintBuild();
      ...
      view: () => ui.view,
      setView: (v) => { ui.view = v; if (v !== 'room') ui.context = null; if (ui.tool.kind !== 'select' && ui.tool.kind !== 'pick' && ui.tool.kind !== 'bulldoze' && ui.tool.kind !== 'area') ui.tool = { kind: 'select' }; if (isPhone() && ui.sheet === 'peek') ui.sheet = 'half'; paintBuild(); },
      context: () => ui.context,
      setContext: (c) => setBuildContext(c),
      sheet: () => ui.sheet,
      setSheet: (size) => {
    ...
    paletteHost.replaceChildren(ui.view === 'room' && ui.context ? roomMenu(host, ui.context) : ui.view === 'rooms' ? roomChooser(host) : buildPalette(host));
    if (sameView) {
      const tray = paletteHost.querySelector('.br-tray');
      if (tray) tray.scrollLeft = trayLeft;
      const cats = paletteHost.querySelector('.br-cats');
      if (cats) cats.scrollLeft = catsLeft;
    }
  ...
  /** One line: size · first effect · running cost. */
  const itemMeta = (def: ObjDef | undefined): string => {
    if (!def) return '';
    const fx = effectLines(def)[0]?.text;
    return [`${def.w}×${def.h} m`, fx, def.upkeep ? `${money(def.upkeep)}/month` : ''].filter(Boolean).join(' · ');
  };
  let detailKey = '';
  const paintDetail = (ok = false, status = '', tone = 'info'): void => {
    const t = ui.tool;
    // Not while an object is being dragged with the mouse: the panel must not cover where it is dropped.
    const def = ui.build && (!isPhone() || isMobileLandscape()) && (t.kind === 'place' || (t.kind === 'move' && !t.dragging)) ? OBJ_BY_ID[t.defId] : undefined;
    if (!def) {
      if (detailKey) { detailKey = ''; detailPanel.replaceChildren(); detailPanel.classList.remove('open'); }
      return;
    }
    const canDo = ok && !def.line && !!ghost();
    const key = `${def.id}|${t.kind}|${ok}|${status}|${canDo}|${ctx.state.cash >= def.cost}`;
    if (key === detailKey) return;
    detailKey = key;
    const reason = unlockReason(ctx.state, def, loc);
    const rooms = allowedRooms(def.id).map((r) => BUILD_ROOM_BY_ID[r].name);
    const place = placementOf(def);
    detailPanel.replaceChildren(
      h('div', { class: 'wd-head' }, h('span', { class: 'wd-art', style: `--c:${def.color}`, text: def.icon }),
        h('div', { class: 'wd-titles' }, h('div', { class: 'wd-name', text: t.kind === 'move' ? `Moving: ${def.name}` : def.name }), h('div', { class: 'wd-desc', text: def.description }))),
      h('div', { class: 'wd-row' }, h('span', { class: 'wd-size', text: `${def.w} × ${def.h}` }), h('span', { class: `wd-price${ctx.state.cash < def.cost ? ' bad' : ''}`, text: t.kind === 'move' ? 'Move' : money(def.cost) })),
      h('div', { class: 'wd-facts' },
        ...effectLines(def).slice(0, 4).map((l) => h('div', { class: 'wd-fx good', text: l.text })),
        h('div', { class: 'wd-fact' }, h('span', { text: 'Upkeep' }), h('strong', { text: `${money(def.upkeep)}/month${def.power ? ` · ${money(def.power)}/day power` : ''}` })),
        h('div', { class: 'wd-fact' }, h('span', { text: 'Placement' }), h('strong', { text: place === 'wall' ? 'Against a wall' : place === 'line' ? 'Drawn as a line' : 'On the floor' })),
        rooms.length ? h('div', { class: 'wd-fact' }, h('span', { text: 'Rooms' }), h('strong', { text: rooms.join(', ') })) : null,
        h('div', { class: 'wd-fact' }, h('span', { text: 'Required' }), h('strong', { class: reason ? 'bad' : '', text: reason ?? 'Available' }))),
      h('div', { class: `wd-status ${tone}` }, icon(tone === 'good' ? 'check' : tone === 'bad' ? 'warning' : 'info', 14), h('span', { text: status })),
      h('button', { class: 'btn primary block wd-place', data: { dp: 'place' }, disabled: !canDo, on: { click: () => commitPlace() } }, icon('check', 15), t.kind === 'move' ? 'Move here' : 'Place'),
      h('div', { class: 'wd-keys' },
        h('button', { class: 'wd-key', data: { dp: 'rotate' }, on: { click: () => rotateTool() } }, h('kbd', { text: 'R' }), 'Rotate'),
        h('div', { class: 'wd-key static' }, h('kbd', { text: '⇱' }), 'Move with the mouse · click to place'),
        h('button', { class: 'wd-key', data: { dp: 'cancel' }, on: { click: () => { cancelTool(); collapsePalette(false); } } }, h('kbd', { text: 'Esc' }), 'Cancel')));
  ...
  const paintConfirm = (): void => {
    ...
    if (t.kind === 'place' || t.kind === 'move') {
      ...
      else if (gh && !gh.ok) { status = (gh.label ?? 'Can’t go here').replace(/^[✕✓]\s*(Blocked:\s*)?/, ''); tone = 'bad'; }
    ...
    confirmBar.replaceChildren(h('div', { class: 'wc-main' },
      ...
      h('div', { class: 'wc-texts' },
        ...
        (t.kind === 'place' || t.kind === 'move') ? h('span', { class: 'wc-meta', text: itemMeta(OBJ_BY_ID[t.defId]) }) : null)));
    ...
    if (roomRect) {
      ...
      const chips = h('div', { class: 'wc-rooms scroll-x' }, ...order.map((c) => h('button', {
        ...
        on: { click: () => {
          ...
          if (r.ok && ui.view !== 'catalog') {
            const cx = Math.floor((roomRect.x0 + roomRect.x1) / 2);
            const cy = Math.floor((roomRect.y0 + roomRect.y1) / 2);
            confirmKey = '';
            afterChange();
            setBuildContext(contextAt(loc, cx, cy, zoneAt(loc.lot, cx, cy)));
            collapsePalette(false);
            return;
          }
  ...
  const onKeyDown = (e: KeyboardEvent): void => {
    ...
    if (mod) return;
    if (e.key === 'Enter' && ui.build && (ui.tool.kind === 'place' || ui.tool.kind === 'move') && ghost()) { e.preventDefault(); commitPlace(); return; }
  ...
  // Right-click cancels placing on the details panel too, as it does on the map.
  detailPanel.addEventListener('contextmenu', (e) => { e.preventDefault(); if (ui.tool.kind !== 'select') { cancelTool(); collapsePalette(false); } });
  document.addEventListener('keydown', onKeyDown);
  document.addEventListener('keyup', onKeyUp);
  window.addEventListener('blur', onBlur);
  // Turning the phone: the context card and build details move to their new places.
  const onLayout = (): void => {
    if (pop.classList.contains('open')) paintPop();
    if (ui.build) paintBuild();
  };
  window.addEventListener('layoutchange', onLayout);
  unsubs.push(() => window.removeEventListener('layoutchange', onLayout));
