...
import { canPlace, carDropCheck, carSize, duplicateObject, footprint, lineCheck, lotStats, moveObject, paintZone, placeLine, placeObject, placeVehicle, removeObject, zoneAt, zoneCost } from '../../sim/lot';
...
import type { CarGhost, Ghost, LineGhost, PaintPreview, Selection } from '../world/render';
...
import type { Filter, Tool } from '../world/build';
...
import { redo, track as trackHistory, undo } from '../world/history';
...
const ui: Ui = { build: false, tool: { kind: 'select' }, category: 'zones', search: '', filter: 'all', styleTarget: 'all', grid: true, snap: true, panel: null, selected: null, moveCar: null, flow: false, cams: {} };
...
export function worldView(ctx: Ctx): View {
  ...
  const fitLot = (): void => {
    ...
    cam.fit(-1, -1, loc.lot.w + 1, loc.lot.h + 4.5, {
      top: phone ? 56 : 64,
      bottom: phone ? (ui.build ? Math.round((paletteHost.offsetHeight || cam.height * 0.52) + 8) : 86) : 92,
      left: !phone && ui.build ? (paletteHost.offsetWidth || 340) + 12 : 16,
  ...
  const ghost = (): Ghost | null => {
    ...
    return { defId: t.defId, x, y, rot: t.rot, ok: check.ok, warn: !!warn, label: check.ok ? `${def.name} · ${money(fee)}${warn ? ` — ${warn}` : ''}` : check.reason };
  };
  ...
  const commitPlace = (): void => {
    ...
    if (!gh.ok) {
      ...
      play('error');
      if (t.kind === 'move' && t.dragging) { ui.tool = { kind: 'select' }; select({ kind: 'object', id: t.objId }); }
      return;
    }
    if (t.kind === 'place') {
      feedback(track(`Place ${def.name.toLowerCase()}`, () => placeObject(ctx.state, loc, t.defId, gh.x, gh.y, gh.rot)));
      ...
      feedback(r, 'click');
      ui.tool = { kind: 'select' };
      if (r.ok) select({ kind: 'object', id: t.objId });
    }
    afterChange();
  };
  ...
  const rotateTool = (): void => {
    const t = ui.tool;
    if (t.kind === 'place' || t.kind === 'move' || t.kind === 'car') { t.rot = t.rot ? 0 : 1; paintConfirm(); return; }
  ...
  const deleteObject = async (id: string): Promise<void> => {
    ...
    if (!o) return;
    const def = OBJ_BY_ID[o.defId];
    if (def.cost >= 2000 && !(await confirmDialog(`Remove the ${def.name.toLowerCase()}?`, `You get ${money(Math.round(def.cost * 0.5))} back. You can undo this.`, 'Remove', true))) return;
    const r = track(`Remove ${def.name.toLowerCase()}`, () => removeObject(ctx.state, loc, id));
    feedback(r, 'click');
    if (r.ok) { ui.selected = null; closePop(); }
    afterChange();
  };
  const duplicate = (id: string): void => {
    ...
    if (!o) return;
    const def = OBJ_BY_ID[o.defId];
    const r = track(`Copy ${def.name.toLowerCase()}`, () => duplicateObject(ctx.state, loc, id));
    feedback(r);
    if (r.ok && r.id) select({ kind: 'object', id: r.id });
    afterChange();
  };
  ...
  const undoAct = (): void => { ctx.act(undo(ctx.state)); warnCache.clear(); afterChange(); };
  const redoAct = (): void => { ctx.act(redo(ctx.state)); warnCache.clear(); afterChange(); };
  ...
  // -------------------------------------------------------------- gestures --
  type Gesture = 'none' | 'pan' | 'pinch' | 'paint' | 'ghost' | 'line' | 'pressObj' | 'pressCar' | 'dragObj' | 'dragCar' | 'armCar';
  ...
  const onDown = (e: PointerEvent): void => {
    ...
    if (pointers.size === 2) {
      ...
      const t = ui.tool;
      if (gesture === 'paint' && t.kind === 'paint') { t.start = undefined; t.end = undefined; }
    ...
    longTimer = window.setTimeout(() => {
      if (moved || pointers.size !== 1 || gesture === 'paint' || gesture === 'ghost' || gesture === 'line' || gesture === 'dragObj' || gesture === 'dragCar') return;
      longFired = true;
      onLongPress(w);
    }, 480);
  };
  ...
  const onMove = (e: PointerEvent): void => {
    ...
    if (!rec) {
      ...
      if (ui.tool.kind === 'car' || (ui.tool.kind === 'place' && !ui.tool.line)) paintConfirm();
    ...
    if (gesture === 'paint' && ui.tool.kind === 'paint') { ui.tool.end = tileAt(hover); return; }
  ...
  const placeConfirm = (): void => {
    confirmBar.style.bottom = isPhone() && ui.build ? `${paletteHost.offsetHeight + 8}px` : '';
  };
  ...
  const paintConfirm = (): void => {
    ...
    if (!ui.build || t.kind === 'select') {
      if (confirmKey) { confirmBar.replaceChildren(); confirmBar.classList.remove('open'); confirmKey = ''; }
      return;
    }
    const phone = isPhone();
    let label = '';
    ...
    if (t.kind === 'place' || t.kind === 'move') {
      ...
      if (def.line && t.kind === 'place') label += phone ? ' — drag a line on the map' : ' — drag to draw a line, right-click to stop';
      else if (gh && !gh.ok) label += ` — ${gh.label}`;
      else if (gh?.warn) label += ` — ${gh.label?.split(' — ')[1] ?? ''}`;
      ok = !!gh && gh.ok;
      doIt = gh && !def.line && (phone || t.x !== undefined) && !(t.kind === 'move' && t.dragging) ? commitPlace : null;
      doLabel = t.kind === 'move' ? 'Move here' : 'Place';
      if (!phone && t.x === undefined && !def.line) label += ' — click to place · R rotate · right-click to stop';
    } else if (t.kind === 'car') {
      const g = carGhost();
      const v = ctx.state.vehicles.find((x) => x.id === t.vehicleId);
      label = g ? g.label : `${v ? vehicleName(v) : 'Car'} — ${phone ? 'tap where it should stand' : 'click where it should stand · R rotate'}`;
      ok = !!g && g.ok;
    ...
    const key = `${label}|${ok}|${!!doIt}|${doLabel}`;
    if (key === confirmKey) return;
    confirmKey = key;
    confirmBar.replaceChildren(h('span', { class: 'wc-label', text: label }));
    if (t.kind === 'place' || t.kind === 'move' || t.kind === 'car') confirmBar.appendChild(h('button', { class: 'btn small', title: 'Rotate (R)', data: { cb: 'rotate' }, on: { click: rotateTool } }, icon('rotate', 14), phone ? '' : 'Rotate'));
    if (doIt) confirmBar.appendChild(h('button', { class: 'btn small primary', data: { cb: 'ok' }, disabled: !ok, on: { click: doIt } }, icon('check', 14), doLabel));
    confirmBar.appendChild(h('button', { class: 'btn small ghost', data: { cb: 'stop' }, on: { click: () => { cancelTool(); collapsePalette(false); } } }, icon('close', 14), phone ? '' : 'Stop'));
    ...
    for (const c of waiting) {
      ...
      el.appendChild(h('div', { class: 'wp-cust' },
        ...
        h('div', { class: 'wm-actions' },
          ...
          h('button', { class: 'btn small', disabled: !td.ok, title: td.reason ?? '', on: { click: () => openTestDrive(ctx, c.id, (car) => crowd.testDrive(loc, car)) } }, icon('key', 14), 'Test drive'),
  ...
  const hudMission = chip('', 'Closest mission', () => ctx.go('missions'));
  const hudService = chip('', 'Workshop jobs today', () => ctx.go('service'));
  const hudFleet = chip('', 'Fleet orders', () => ctx.go('fleet'));
  hud.append(hudOpen, hudDecision, hudToday, hudCust, hudCars, hudTime, hudRep, hudMission, hudService, hudFleet, hudIssues);
  ...
  const paintHud = (): void => {
    ...
    setChip(hudDecision, String(dec), h('span', { text: '⚖️' }), h('span', { class: 'wh-v', text: dec > 1 ? `${dec} decisions` : 'Decision' }));
    ...
    if (best) setChip(hudMission, `${best.m.id}|${Math.round(best.p * 20)}`, h('span', { text: best.m.icon }), h('span', { class: 'wh-v', text: `${Math.round(best.p * 100)}%` }));
    ...
    setChip(hudFleet, String(fleetOpen), h('span', { text: '🚐' }), h('span', { class: 'wh-v', text: String(fleetOpen) }));
  ...
  const onKeyDown = (e: KeyboardEvent): void => {
    ...
    else if ((e.code === 'Delete' || e.code === 'Backspace') && ui.build && sel) {
      e.preventDefault();
      void deleteObject(sel);
    }
