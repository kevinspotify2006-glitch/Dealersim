...
import { conditionTag, demandTag, pageHead, riskTag, segmented, statusTag, vehicleCard, helpButton } from '../kit';
  ...
  function renderList(): void {
    ...
    if (isWide()) {
      const rows = list.map((v) => {
        const m = margin(v);
        return [
          h('div', { class: 'cell-car' }, h('span', { class: 'mini-dot', style: `background:${v.colorHex}` }), h('div', {}, h('div', { class: 'cell-person-name', text: `${v.year} ${v.brand} ${v.model}` }), h('div', { class: 'tiny muted', text: `${v.trim} · ${v.fuel} · ${Math.round(v.mileage / 1000)}k km` }))),
          statusTag(v.status),
          conditionTag(v),
