...
import { releasePrice } from '../../sim/systems/automation';
import { modeFor } from '../../sim/systems/autocore';
...
export function openVehicle(ctx: Ctx, vehicleId: string, startTab: Tab = 'overview'): void {
  ...
  const pricing = (v: Vehicle): HTMLElement => {
    ...
    // Inventory automation reprices cars — except the ones you priced by hand.
    if (modeFor(state, v.locationId, 'inventory') !== 'manual') {
      wrap.appendChild(h('div', { class: 'vm-autoprice' },
        h('span', { class: 'tiny muted', text: v.manualPrice ? 'Priced by hand: inventory automation leaves this price alone.' : 'Inventory automation prices and reprices this car. Saving a price keeps it yours.' }),
        v.manualPrice ? h('button', { class: 'btn small', data: { act: 'auto-price' }, on: { click: () => run(releasePrice(state, v.id)) } }, 'Let automation price it') : null));
    }
