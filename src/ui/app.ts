...
import { haptic, handleBack, pushBackHandler, setHaptics, setRouteBack, setSaveHook } from '../platform/platform';
...
export const ROUTE_ALIASES: Record<string, { route: string; params: Record<string, string> }> = {
  inventory: { route: 'dealership', params: { panel: 'cars' } },
  customers: { route: 'dealership', params: { panel: 'customers' } },
  staff: { route: 'dealership', params: { panel: 'staff' } },
  finances: { route: 'dealership', params: { panel: 'finances' } },
  build: { route: 'dealership', params: { build: '1' } },
  upgrades: { route: 'company', params: { tab: 'services' } },
  expansion: { route: 'company', params: { tab: 'locations' } },
  marketing: { route: 'company', params: { tab: 'marketing' } },
  dashboard: { route: 'reports', params: { tab: 'overview' } },
  sales: { route: 'reports', params: { tab: 'sales' } },
};
...
export class App {
  ...
  start(): void {
    ...
    this.updateTimer = window.setInterval(() => {
      ...
      this.engine.hold = !!document.querySelector('.modal-overlay');
  ...
  private build(): void {
    ...
    const shell = h('div', { class: 'shell' }, brand, this.topbar, this.navHost, this.main, this.mobileNavHost);
  ...
  private renderNav(): void {
    ...
    for (const section of SECTIONS) {
      ...
      if (this.navOpen) group.appendChild(h('div', { class: 'nav-group-head', text: section.label }));
  ...
  private renderTopbar(): void {
    ...
    const metric = (label: string, id: string, tip: string): HTMLElement => tipped(h('div', { class: 'metric' },
      h('span', { class: 'metric-label', text: label }),
      h('span', { class: 'metric-value', id }),
      h('span', { class: 'metric-sub', id: `${id}-sub` })), tip);
    hud.appendChild(metric('Cash', 'hud-cash', 'Money in the bank right now.'));
    hud.appendChild(metric('Today', 'hud-today', 'Cars sold today and today\'s result so far (gross profit on sales minus running costs).'));
    hud.appendChild(metric('Reputation', 'hud-rep', 'What customers think of you (0–100).'));
    ...
    tipped(waiting, 'Customers waiting to be served. Serve them yourself or your salespeople will step in when they run out of patience.');
  ...
  private updateChrome(): void {
    ...
    set('hud-today', `${state.today.sold} sold`);
    sub('hud-today', `${today >= 0 ? '+' : '−'}${moneyShort(Math.abs(today))} · L${state.companyLevel} ${levelDef(state.companyLevel).name}`);
    void companyValue;
    ...
    if (pill && pillN) {
      pillN.textContent = String(waiting);
    ...
    if (brandCompany) brandCompany.textContent = `${state.companyName} · ${activeLocation(state).name}`;
  ...
  private openNotices(): void {
    const { body } = modal({ title: 'Notifications', width: 620 });
    const notices = this.state.notices;
    if (!notices.length) body.appendChild(h('p', { class: 'empty', text: 'Nothing new. Sales, arrivals, events and warnings will show up here.' }));
    for (const n of notices.slice(0, 60)) {
      body.appendChild(h('div', { class: `alert-row notice-${n.kind}${n.read ? '' : ' unread'}` },
