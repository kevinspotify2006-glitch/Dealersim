...
import { MORE_SECTIONS, SECTIONS, sectionOfRoute } from './nav';
...
export const ROUTE_ALIASES: Record<string, { route: string; params: Record<string, string>; tabs?: Record<string, string> }> = {
  ...
  automation: { route: 'management', params: { tab: 'automation' } },
  headhunter: { route: 'management', params: { tab: 'headhunter' } },
  hr: { route: 'management', params: { tab: 'hr' } },
  approvals: { route: 'management', params: { tab: 'overview' } },
};
...
export class App {
  ...
  private renderNav(): void {
    ...
    if (this.openSection === 'more') {
      const entries = MORE_SECTIONS.flatMap((id) => this.screensIn(SECTIONS.find((x) => x.id === id)!));
      const unread = this.state.notices.filter((n) => !n.read).length;
      const item = (key: string, ic: string, label: string, sub: string, count: number, run: () => void, active = false): HTMLElement => {
        const glyph = h('span', { class: 'nav-icon' });
        glyph.appendChild(icon(ic, 18));
        return h('button', { class: `nav-item more-item${active ? ' active' : ''}`, data: { more: key }, on: { click: run } },
          glyph, h('span', { class: 'more-txt' }, h('span', { class: 'nav-label', text: label }), h('span', { class: 'more-sub', text: sub })), count > 0 ? h('span', { class: 'nav-badge', text: String(count) }) : null);
      };
      this.mobileNavHost.appendChild(h('div', { class: 'nav-sheet more-sheet' },
        h('div', { class: 'nav-sheet-head' }, h('span', { text: 'More' }),
          h('button', { class: 'icon-btn', aria: { label: 'Close' }, on: { click: () => { this.closeSheet(); this.renderNav(); } } }, icon('close', 15))),
        h('div', { class: 'nav-sheet-body' },
          ...entries.map((e) => item(e.route, e.icon, e.label, e.blurb, e.badge?.(this.state) ?? 0, () => this.go(e.route), e.route === this.route)),
          item('notices', 'bell', 'Notifications', 'Filter by critical, important, info and automation.', unread, () => { this.closeSheet(); this.renderNav(); this.openNotices(); }),
          item('status', 'dashboard', 'Dealership status', 'Cash, today, stock, team and the clock in one sheet.', 0, () => { this.closeSheet(); this.renderNav(); this.openStatus(); }))));
    }
    const bar = h('div', { class: 'nav-bar' });
    for (const def of SECTIONS) {
      // Phones: five play sections in the bar; Management and Settings live under More.
      if (MORE_SECTIONS.includes(def.id)) continue;
    ...
    {
      const inMore = here ? MORE_SECTIONS.includes(here.id) : false;
      const count = MORE_SECTIONS.reduce((n, id) => n + this.screensIn(SECTIONS.find((x) => x.id === id)!).reduce((m, e) => m + (e.badge?.(this.state) ?? 0), 0), 0);
      const glyph = h('span', { class: 'nav-icon' });
      glyph.appendChild(icon('more', 20));
      bar.appendChild(h('button', {
        class: `nav-item${inMore ? ' active' : ''}${this.openSection === 'more' ? ' expanded' : ''}`,
        data: { section: 'more' },
        aria: { label: 'More' },
        on: {
          click: () => {
            if (this.openSection === 'more') this.closeSheet();
            else {
              this.closeSheet();
              this.openSection = 'more';
              this.sheetRelease = pushBackHandler(() => { this.closeSheet(); this.renderNav(); return true; });
            }
            this.renderNav();
          },
        },
      }, glyph, h('span', { class: 'nav-label', text: 'More' }), count > 0 ? h('span', { class: 'nav-badge', text: String(count) }) : null));
    }
  ...
  private renderBadges(): void {
    for (const host of [this.navHost, this.mobileNavHost]) {
      ...
      const more = host.querySelector('.nav-bar .nav-item[data-section="more"]');
      if (more) paintBadge(more, MORE_SECTIONS.reduce((n, id) => n + this.screensIn(SECTIONS.find((x) => x.id === id)!).reduce((m, e) => m + (e.badge?.(this.state) ?? 0), 0), 0));
    }
  }
  ...
  private noticeFilter: 'all' | 'critical' | 'important' | 'info' | 'auto' = 'all';
  private noticeGroup = true;

  /** Notification centre: filter by weight or automation, and group repeats. */
  private openNotices(): void {
    ...
    type N = (typeof notices)[number];
    const weight = (n: N): 'critical' | 'important' | 'info' => (n.kind === 'bad' ? 'critical' : n.kind === 'event' || n.kind === 'sale' || (n.kind === 'good' && n.action) ? 'important' : 'info');
    const match = (n: N, f: typeof this.noticeFilter): boolean => f === 'all' || (f === 'auto' ? n.tag === 'auto' : weight(n) === f);
    const KIND_ICON: Record<string, string> = { bad: 'critical', event: 'warning', good: 'good', sale: 'cash', info: 'info' };
    const clean = (t: string): string => t.replace(/^[^\p{L}\p{N}€+−-]+\s*/u, '');
    const row = (n: N, count = 1): HTMLElement => h('div', { class: `alert-row notice-${n.kind}${unreadIds.has(n.id) ? ' unread' : ''}`, data: { notice: String(n.id), weight: weight(n), ...(n.tag ? { tag: n.tag } : {}) } },
      h('span', { class: `notice-ic ${n.kind}` }, icon(n.tag === 'auto' ? 'gauge' : KIND_ICON[n.kind] ?? 'info', 16)),
      h('div', { style: 'flex:1;min-width:0' }, h('div', { class: 'alert-title', text: clean(n.text) })),
      count > 1 ? h('span', { class: 'notice-count', text: `×${count}` }) : null,
      h('span', { class: 'alert-time', text: n.day === this.state.day ? 'Today' : `Day ${n.day}` }));
    // Similar notices on the same day ("X arrived at Y", "Z sold the …") fold into one row.
    const stem = (n: N): string => `${n.day}|${n.kind}|${n.tag ?? ''}|${clean(n.text).replace(/[0-9€.,]+/g, '#').split(' ').slice(-4).join(' ')}`;
    const render = (): void => {
      const f = this.noticeFilter;
      const count = (k: typeof f): number => notices.filter((n) => match(n, k)).length;
      const chips = h('div', { class: 'notice-filters pill-row' },
        ...(['all', 'critical', 'important', 'info', 'auto'] as const).map((k) => h('button', { class: `pill${f === k ? ' active' : ''}`, data: { nfilter: k }, on: { click: () => { this.noticeFilter = k; render(); } } },
          k === 'all' ? 'All' : k === 'critical' ? 'Critical' : k === 'important' ? 'Important' : k === 'info' ? 'Info' : 'Automation', h('span', { class: 'seg-count', text: String(count(k)) }))),
        h('button', { class: `pill${this.noticeGroup ? ' active' : ''}`, data: { ngroup: '1' }, on: { click: () => { this.noticeGroup = !this.noticeGroup; render(); } } }, 'Group similar'));
      const list = notices.filter((n) => match(n, f)).slice(0, 120);
      const out: HTMLElement[] = [chips];
      if (!list.length) {
        out.push(h('div', { class: 'empty-state' }, h('span', { class: 'empty-ic' }, icon('bell', 26)),
          h('div', { class: 't-section', text: notices.length ? 'Nothing in this filter' : 'All caught up' }),
          h('p', { class: 't-secondary', text: 'Sales, arrivals, events, warnings and your managers\' reports show up here.' })));
      }
      const section = (label: string, items: N[]): void => {
        if (!items.length) return;
        out.push(h('div', { class: 't-label', text: `${label} (${items.length})` }));
        if (!this.noticeGroup) { out.push(...items.map((n) => row(n))); return; }
        const groups = new Map<string, N[]>();
        for (const n of items) { const k = stem(n); const g = groups.get(k); if (g) g.push(n); else groups.set(k, [n]); }
        for (const g of groups.values()) {
          if (g.length === 1) { out.push(row(g[0])); continue; }
          const d = h('details', { class: 'notice-group' }, h('summary', {}, row(g[0], g.length)), ...g.slice(1).map((n) => row(n)));
          out.push(d);
        }
      };
      section('New', list.filter((n) => unreadIds.has(n.id)));
      section('Earlier', list.filter((n) => !unreadIds.has(n.id)));
