...
export class App {
  ...
  private renderTopbar(): void {
    ...
    const chips: [string, string, string, () => void][] = [
      ['💰', 'cash', 'Cash in the bank, and this month\'s result.', () => this.go('business', { tab: 'finances' })],
      ['📈', 'today', 'Today: the result so far and cars sold.', () => this.go('business', { tab: 'overview' })],
      ['🚘', 'stock', 'Cars in stock (arriving, being prepared, for sale).', () => this.go('inventory', { tab: 'stock' })],
      ['👥', 'staff', 'Employees (and how many are at work today).', () => this.go('people', { tab: 'staff' })],
      ['🔧', 'service', 'Workshop jobs today and open service work.', () => this.go('service', { tab: 'planning' })],
      ['⭐', 'rep', 'Reputation (0–100).', () => this.go('business', { tab: 'company' })],
    ];
    ...
    const waiting = h('button', { class: 'waiting-pill', id: 'hud-waiting', on: { click: () => this.go('dealership', { panel: 'customers' }) } }, h('span', { text: '👤' }), h('span', { id: 'hud-waiting-n', text: '0' }));
