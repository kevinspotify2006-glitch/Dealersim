...
export interface Scene {
  ...
  /** Build Mode 2.0: the room being built in — outlined, the rest dimmed a little. */
...
export function renderScene(canvas: HTMLCanvasElement, s: Scene): void {
  ...
  if (s.focus) {
    const f = s.focus;
    const x0 = f.x0; const y0 = f.y0; const x1 = f.x1 + 1; const y1 = f.y1 + 1;
    const W0 = -60; const H0 = -60; const W1 = lot.w + 60; const H1 = lot.h + 60;
    g.fillStyle = 'rgba(4,8,14,0.34)';
    g.fillRect(W0, H0, W1 - W0, y0 - H0);
    g.fillRect(W0, y1, W1 - W0, H1 - y1);
    g.fillRect(W0, y0, x0 - W0, y1 - y0);
    g.fillRect(x1, y0, W1 - x1, y1 - y0);
    g.strokeStyle = '#3c9dff';
    g.lineWidth = 0.14;
    g.strokeRect(x0 - 0.05, y0 - 0.05, x1 - x0 + 0.1, y1 - y0 + 0.1);
    g.strokeStyle = 'rgba(60,157,255,0.35)';
    g.lineWidth = 0.4;
    g.strokeRect(x0 - 0.25, y0 - 0.25, x1 - x0 + 0.5, y1 - y0 + 0.5);
  }
