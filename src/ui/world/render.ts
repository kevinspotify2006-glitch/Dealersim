...
import { footprint, isIndoor, lotStats, zoneAt } from '../../sim/lot';
...
export interface Ghost { defId: string; x: number; y: number; rot: 0 | 1; ok: boolean; label?: string }
...
export function floorColor(code: ZoneCode, lot: Lot): string {
  const floor = FLOOR_STYLES.find((s) => s.id === lot.style.floor) ?? FLOOR_STYLES[0];
  switch (code) {
    case 's': return floor.color;
    case 'r': return mix(floor.color, '#b9a88f', 0.35);
    case 'l': return mix(floor.color, '#8a6a4a', 0.45);
    case 'o': return mix(floor.color, '#51606f', 0.5);
    case 'w': return '#5b5f66';
    case 'd': return '#3e5968';
  ...
  for (let y = 0; y < lot.h; y += 1) {
    for (let x = 0; x < lot.w; x += 1) {
      ...
      if (z === 'a') {
        ...
        if (floor.id === 'tile' || floor.id === 'premium' || z === 'w' || z === 'd') {
...
function styleKey(lot: Lot, state: GameState): string {
  return `${lot.style.floor}|${lot.style.walls}|${lot.style.lighting}|${state.companyName}`;
}
...
function layer(o: LotObject): number {
  ...
  if (def.category === 'exterior' && (def.id === 'tree' || def.id === 'pylon' || def.id === 'lamp' || def.id === 'flag')) return 8;
  return 4;
}
...
function drawWalls(g: CanvasRenderingContext2D, lot: Lot): void {
  const wall = WALL_STYLES.find((s) => s.id === lot.style.walls) ?? WALL_STYLES[0];
  const doors = new Set<number>();
  const windows = new Set<number>();
  for (const o of lot.objects) {
    if (o.defId !== 'door' && o.defId !== 'window') continue;
    const f = footprint(o);
    for (let y = f.y; y < f.y + f.h; y += 1) for (let x = f.x; x < f.x + f.w; x += 1) (o.defId === 'door' ? doors : windows).add(y * lot.w + x);
  }
  const T = 0.24;
  const segs: { x: number; y: number; w: number; h: number; kind: 'wall' | 'glass' | 'door' }[] = [];
  ...
  const edge = (ax: number, ay: number, bx: number, by: number, vertical: boolean): void => {
    ...
    if (vertical) segs.push({ x: bx - T / 2, y: by - 0.02, w: T, h: 1.04, kind });
    else segs.push({ x: bx - 0.02, y: by - T / 2, w: 1.04, h: T, kind });
  };
  ...
  if (wall.id === 'glass') {
    ...
    for (const s of segs) if (s.kind === 'wall') g.fillRect(s.x + s.w * 0.4, s.y + s.h * 0.4, Math.max(0.03, s.w * 0.2), Math.max(0.03, s.h * 0.2));
  }
}
...
export function drawObject(g: CanvasRenderingContext2D, o: LotObject, loc: Location | null, state: GameState | null, t: number, ghost: boolean): void {
  ...
  switch (def.id) {
    case 'parking':
    case 'storage': {
      g.strokeStyle = def.id === 'parking' ? 'rgba(238,241,244,0.85)' : 'rgba(210,190,120,0.55)';
      g.lineWidth = 0.1;
      g.beginPath();
      if (h >= w) {
    ...
    case 'display': {
      ...
      grd.addColorStop(0, 'rgba(255,154,77,0.28)');
      grd.addColorStop(1, 'rgba(255,154,77,0.02)');
      g.fillStyle = grd;
      roundRect(g, x + 0.1, y + 0.1, w - 0.2, h - 0.2, 0.5);
      g.fill();
      g.strokeStyle = 'rgba(255,154,77,0.9)';
      g.lineWidth = 0.07;
      g.stroke();
      break;
    }
    ...
    case 'charger': {
      ...
      g.fillStyle = '#2fd18b';
      g.fillRect(x + 0.35, y + 0.25, 0.3, 0.2);
      break;
    }
    case 'lift': {
    ...
    case 'flag': {
      ...
      g.fillStyle = ((o.x + o.y) % 2 ? '#3cc7ff' : '#ff7a1a');
    ...
    default: {
      shadow();
      g.fillStyle = def.color;
      roundRect(g, x + 0.08, y + 0.08, w - 0.16, h - 0.16, 0.15); g.fill();
      g.fillStyle = 'rgba(255,255,255,0.18)';
      g.fillRect(x + 0.2, y + 0.2, w - 0.4, 0.12);
    }
  }
  g.restore();
  void loc; void t;
...
export function drawCar(g: CanvasRenderingContext2D, x: number, y: number, angle: number, color: string, body: string, alpha = 1): void {
  const long = body === 'Van' || body === 'Pickup' ? 4.7 : body === 'SUV' || body === 'Estate' ? 4.4 : body === 'Coupe' || body === 'Convertible' ? 4.2 : 4.0;
  const wide = body === 'Van' || body === 'SUV' || body === 'Pickup' ? 1.9 : 1.78;
  g.save();
  g.globalAlpha = alpha;
  g.translate(x, y);
  g.rotate(angle);
...
export function renderScene(canvas: HTMLCanvasElement, s: Scene): void {
  ...
  if (s.ghost) {
    ...
    if (def) {
      ...
      g.fillStyle = gh.ok ? 'rgba(47,209,139,0.22)' : 'rgba(255,77,94,0.28)';
      g.fillRect(gh.x, gh.y, w, h);
      g.globalAlpha = 0.75;
      drawObject(g, { id: 'ghost', defId: gh.defId, x: gh.x, y: gh.y, rot: gh.rot }, loc, state, s.time, true);
      g.globalAlpha = 1;
      g.strokeStyle = gh.ok ? '#2fd18b' : '#ff4d5e';
      g.lineWidth = 0.1;
      g.strokeRect(gh.x, gh.y, w, h);
      if (gh.label) hud.push({ label: gh.label, x: gh.x + w / 2, y: gh.y - 0.6, tone: gh.ok ? 'good' : 'bad' });
    }
  }
...
function drawHud(g: CanvasRenderingContext2D, items: Hud[], cam: Camera, toScreen: (x: number, y: number) => { x: number; y: number }): void {
  ...
  for (const it of items) {
    ...
    const text = tiny ? (it.icon ?? (it.tone === 'price' ? '€' : it.label.slice(0, 1))) : it.icon ? `${it.icon} ${it.label}` : it.label;
