...
export function drawObject(g: CanvasRenderingContext2D, o: LotObject, loc: Location | null, state: GameState | null, t: number, ghost: boolean): void {
  ...
  switch (def.id) {
    ...
    case 'prepspot':
    case 'storage': {
      const tint: Record<string, string> = { parking: 'rgba(238,241,244,0.85)', frontrow: 'rgba(255,194,51,0.95)', occasion: 'rgba(47,209,139,0.9)', evparking: 'rgba(47,209,139,0.9)', visitorparking: 'rgba(60,199,255,0.9)', prepspot: 'rgba(163,171,182,0.7)', storage: 'rgba(210,190,120,0.55)' };
    ...
    default: {
      ...
      g.textAlign = 'center';
      g.textBaseline = 'middle';
      g.fillText(def.icon, cx, cy + 0.04);
    }
  }
  g.restore();
  void loc; void t;
...
export function drawCar(g: CanvasRenderingContext2D, x: number, y: number, angle: number, color: string, body: string, alpha = 1): void {
  const long = body === 'Van' || body === 'Pickup' ? 4.7 : body === 'SUV' || body === 'Crossover' || body === 'Wagon' ? 4.4 : body === 'Coupe' || body === 'Convertible' ? 4.2 : 4.0;
  const wide = body === 'Van' || body === 'SUV' || body === 'Pickup' || body === 'Crossover' ? 1.9 : 1.78;
  g.save();
  g.globalAlpha = alpha;
  g.translate(x, y);
  g.rotate(angle);
