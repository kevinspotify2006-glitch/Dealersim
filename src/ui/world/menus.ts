...
function objectMenu(a: WorldActions, loc: Location, o: LotObject): HTMLElement {
  ...
  actions.appendChild(btn('Duplicate', () => a.duplicate(o.id), '', 'plus'));
  actions.appendChild(btn('Copy', () => a.copy(o.id), 'ghost', 'layers'));
  actions.appendChild(btn(`Delete · +${money(Math.round(def.cost * 0.5))}`, () => a.deleteObject(o.id), 'ghost', 'trash'));
  el.appendChild(actions);
  return el;
}
