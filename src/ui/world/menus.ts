...
export interface WorldActions {
  ...
  /** Build Mode 2.0: open the build menu of the room at this point. */
...
function zoneMenu(a: WorldActions, loc: Location, x: number, y: number): HTMLElement {
  ...
  const actions = h('div', { class: 'wm-actions' });
  if (a.buildAt) actions.appendChild(btn('Build here', () => a.buildAt!(x, y), 'primary', 'hammer'));
  else actions.appendChild(btn('Build here', () => a.build(code === '.' || code === 'a' || code === 'g' ? 'zones' : code === 's' ? 'showroom' : code === 'w' || code === 'd' || code === 'p' ? 'service' : code === 'o' || code === 'm' || code === 'k' ? 'staff' : code === 'l' || code === 'r' || code === 'b' ? 'customers' : code === 'f' ? 'finance' : 'zones'), 'primary', 'hammer'));
