...
export function buildPalette(host: BuildHost): HTMLElement {
  ...
  // Mode: BUILD · MOVE · BULLDOZE · AREA (bulldoze a whole area).
  const mode = modeOf(tool);
  const md = (id: BuildMode, bt: string, label: string, glyph: string, title: string, run: () => void): HTMLElement =>
    h('button', { class: `bp-mode${mode === id ? ' active' : ''}${id === 'bulldoze' || id === 'area' ? ' danger' : ''}`, data: { bt }, title, aria: { label: title, pressed: String(mode === id) }, on: { click: run } }, h('span', { class: 'bp-mode-ic', text: glyph }), h('span', { text: label }));
  root.appendChild(h('div', { class: 'bp-modes', role: 'group', aria: { label: 'Build mode' } },
    md('build', 'select', 'Build', '🏗️', 'Build: place items, tap what is built to select it', () => host.setTool({ kind: 'select' })),
    md('move', 'pick', 'Move', '✋', 'Move: tap anything built to pick it up and drop it somewhere else (M)', () => host.setTool({ kind: 'pick' })),
    md('bulldoze', 'bulldoze', 'Bulldoze', '💥', 'Bulldoze: tap an object to demolish it and get part of its cost back (X · Delete)', () => host.setTool({ kind: 'bulldoze' })),
    md('area', 'area', 'Area', '⬚', 'Bulldoze area: drag a rectangle to demolish everything in it', () => host.setTool({ kind: 'area' }))));
  ...
  const search = h('input', { type: 'search', class: 'bp-search-input', placeholder: 'Search 180+ items… (desk, parking, EV, test drive, <500)', value: host.search(), aria: { label: 'Search build items' } });
...
function itemCard(host: BuildHost, def: ObjDef): HTMLElement {
  ...
  const card = h('button', {
    ...
    disabled: locked,
    title: reason ?? '',
    aria: { label: `${def.name}, ${money(def.cost)}${reason ? `. ${reason}` : ''}` },
  ...
  h('span', { class: 'bp-eff', text: `${def.w}×${def.h}${def.line ? ' · line' : ''}${OBJ_UPGRADES[def.id] ? ' · ⬆️' : ''}${fx ? ` · ${fx.text}` : def.slot ? ' · +1 space' : def.station ? ` · ${def.station[0]} desk` : ''}` }));
...
function body(host: BuildHost, rawCat: string, loc: Location): HTMLElement[] {
  ...
  if (mode === 'bulldoze' || mode === 'area' || tool.kind === 'pick') {
    out.push(h('div', { class: `bp-modehint ${mode}` },
      h('strong', { text: mode === 'bulldoze' ? '💥 Bulldoze' : mode === 'area' ? '⬚ Bulldoze area' : '✋ Move' }),
      ...
      h('button', { class: 'btn small ghost', on: { click: () => host.setTool({ kind: 'select' }) } }, 'Back to building')));
  }
