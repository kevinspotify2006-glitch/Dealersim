...
export interface BuildHost {
  ...
  /** Build Mode 2.0: which view the drawer shows, and the room being built in. */
  view?: () => BuildView;
  setView?: (view: BuildView) => void;
  context?: () => BuildContext | null;
  setContext?: (ctx: BuildContext | null) => void;
}

/** rooms: pick a room · room: that room's categories and items · catalog: every item (search, filters, tools). */
export type BuildView = 'rooms' | 'room' | 'catalog';

/** The room you are building in: its type, category, and (for a real room on the map) where it is. */
...
export function buildPalette(host: BuildHost): HTMLElement {
  ...
  // Back to the room overview (the catalogue is the "everything" view).
  if (host.setView) root.appendChild(h('button', { class: 'bp-backrow', data: { ctx: 'rooms' }, on: { click: () => host.setView!('rooms') } }, icon('back', 15), h('span', { text: 'Rooms' }), h('span', { class: 'bp-backrow-sub', text: 'All items' })));
...
/** Grip, head, the three modes and the tool row: the same on every build view. */
export function paletteTop(host: BuildHost, root: HTMLElement): void {
  const s = host.ctx.state;
...
export function openLocked(host: BuildHost, def: ObjDef, reason: string): void {
  haptic(12);
