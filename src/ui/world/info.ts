...
export function objectInfo(state: GameState, def: ObjDef, placed?: { loc: Location; o: LotObject }): HTMLElement {
  ...
  const el = h('div', { class: 'info-card' },
    h('div', { class: 'ic-head' }, h('span', { class: 'ic-icon', text: def.icon }), h('div', {}, h('div', { class: 'ic-title', text: def.name.toUpperCase() }), h('div', { class: 'ic-sub', text: `${def.category[0].toUpperCase()}${def.category.slice(1)} · ${def.w}×${def.h} m${def.line ? ' · drawn in lines' : ''}` }))),
