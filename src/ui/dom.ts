...
export function attachSheetDrag(card: HTMLElement, head: HTMLElement | null, grip: HTMLElement, dismiss: () => void): void {
  ...
  let dragging = false;
  let startT = 0;
  // Sideways on a phone dialogs are a right-hand drawer, not a sheet: no drag.
  const isSheet = (): boolean => window.matchMedia('(max-width: 900px)').matches && !document.body.classList.contains('lay-landscape');
