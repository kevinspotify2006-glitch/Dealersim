...
export type BuildTabId =
  ...
  | 'storage' | 'decoration' | 'exterior' | 'parking' | 'ev' | 'technology' | 'security' | 'premium'
...
export const BUILD_TABS: BuildTab[] = [
  ...
  { id: 'staff', name: 'Staff', icon: '👥', hint: 'Workstations for every role and things that keep your team happy', test: (d) => !!d.station || fx(d, 'morale', 'management') },
  ...
  { id: 'marketing', name: 'Marketing', icon: '📣', hint: 'Signs, screens, flags and photography: more passers-by and leads', test: (d) => fx(d, 'footfall', 'curb', 'photo') && !tags(d, 'green') || tags(d, 'marketing', 'branding', 'advert', 'reclame', 'logo') },
  { id: 'storage', name: 'Storage', icon: '📦', hint: 'Hidden car storage, parts shelving and cabinets', test: (d) => d.slot === 'storage' || tags(d, 'storage', 'cabinet', 'parts', 'kast') || d.zones.every((z) => z === 'p' || z === 't') },
  ...
  { id: 'exterior', name: 'Exterior', icon: '🌳', hint: 'Everything that stands outside: gates, signs, lighting, greenery', test: (d) => d.category === 'outdoor' || onlyOutdoor(d) },
  { id: 'parking', name: 'Parking', icon: '🅿️', hint: 'Car spaces for sale, storage and visitor parking', test: (d) => d.slot === 'parking' || d.slot === 'storage' || fx(d, 'visitorParking') || tags(d, 'parking') },
  { id: 'ev', name: 'EV', icon: '⚡', hint: 'Chargers, EV displays and EV diagnostics', test: (d) => fx(d, 'ev') || tags(d, 'electric', 'ev', 'charger', 'charging') },
  { id: 'technology', name: 'Technology', icon: '🖥️', hint: 'Computers, screens, diagnostics and systems', test: (d) => tags(d, 'computer', 'technology', 'screen', 'diagnostics', 'crm', 'it', 'software', 'cctv', 'sensor') || fx(d, 'equipment') },
  ...
  { id: 'premium', name: 'Premium', icon: '💎', hint: 'High-end items for luxury and prestige buyers', test: (d) => tags(d, 'luxury', 'premium', 'vip', 'hero') || d.spot?.fit === 'premium' || ((d.minLevel ?? 1) >= 3) || (d.cost >= 7000 && !d.slot) },
  { id: 'style', name: 'Floors & ceilings', icon: '🎨', hint: 'Floors, wall finishes and ceiling lighting per room' },
  { id: 'land', name: 'Land', icon: '🗺️', hint: 'Buy more land around your lot' },
  { id: 'templates', name: 'Templates', icon: '📐', hint: 'Replace the whole layout with a ready-made design' },
];
