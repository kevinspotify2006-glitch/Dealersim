 ...
 */
import type { PricingPolicy, BodyType, Category, ConditionGrade, Drivetrain, FuelType, GameState, Vehicle } from './types';
...
export const PRICING_POLICY: Record<PricingPolicy, { name: string; price: number; interest: number; description: string }> = {
  fast: { name: 'Fast turnover', price: 0.93, interest: 0.12, description: 'Priced to move: cars sell quickly, margins are thin.' },
  ...
  conservative: { name: 'Conservative', price: 1.02, interest: -0.03, description: 'A little above the market: keeps margin, gives ground slowly.' },
  ...
  maxprofit: { name: 'Maximum profit', price: 1.08, interest: -0.11, description: 'Top of the market: every car earns the most, but they wait longer.' },
};
