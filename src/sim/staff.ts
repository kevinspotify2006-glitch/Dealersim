...
export function titleOf(e: Pick<Employee, 'role' | 'level'>): string {
  const name = ROLE_BY_ID[e.role]?.name ?? e.role;
  if (e.level >= 5) return `Lead ${name}`;
  if (e.level >= 3) return `Senior ${name}`;
  return name;
}
...
function makeSkills(role: Role, skill: number): Partial<Record<SkillId, number>> {
...
export function staffDaily(state: GameState): void {
  ...
  for (const e of [...state.employees]) {
    ...
    const managerBoost = managers.reduce((m, x) => Math.max(m, skillOf(x, 'management')), 0) / 100 * 10;
    ...
    const targetStress = clamp(18 + load * 35 + (e.stationId ? 0 : 20) - relief, 0, 100);
...
export function synergies(state: GameState, locationId: string): Synergy[] {
  ...
  return [
    { id: 'salesteam', name: 'Led sales team', icon: '🧭', active: count('manager') >= 1 && count('sales') >= 2, effect: 'Salespeople get 10% more out of every deal.', needs: 'A manager and 2+ salespeople' },
