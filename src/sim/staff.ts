...
const SKILL_PROFILE: Record<Role, Partial<Record<SkillId, number>>> = {
  ...
  hr: { management: 1, service: 0.8, reliability: 0.7, finance: 0.4 },
  logistics: { management: 1, speed: 0.8, reliability: 0.8, negotiation: 0.5 },
};
...
/** v7.3: remember who left so HR can replace them, and drop HR links to them. */
export function noteLeft(state: GameState, e: Employee): void {
  if (state.automation) {
    state.automation.left.push({ role: e.role, locationId: e.locationId, day: state.day });
    if (state.automation.left.length > 40) state.automation.left.splice(0, state.automation.left.length - 40);
  }
  for (const o of state.employees) if (o.hrBy === e.id) o.hrBy = undefined;
}
...
export function staffDaily(state: GameState): void {
  ...
  for (const e of [...state.employees]) {
    ...
    // An HR manager looking after them (return-to-work talks, cover) halves the absence.
    if (gameRng.chance(Math.max(0.002, (100 - rel) / 100 * 0.05 * (e.hrBy ? 0.55 : 1)))) {
