...
import { bump, managerFor, modeFor, requestApproval, salesFloor } from './systems/autocore';
...
function runStaffDeal(state: GameState, c: Customer, v: Vehicle, staffId: string, handled: Record<string, number>): boolean {
  ...
  let price = Math.max(floor, achieved);
  // v7.3 sales policy: minimum margin, maximum discount and approvals.
  const mode = modeFor(state, c.locationId, 'sales');
  const pol = state.automation?.policies.sales;
  if (mode !== 'manual' && pol) {
    const mgr = managerFor(state, c.locationId, 'sales');
    // A weak sales manager now and then lets a deal slip under the policy.
    const slip = mgr && gameRng.chance(mgr.err) ? 0.97 : 1;
    if (slip < 1) bump(state, 'sales', 'errors');
    const cost = totalCost(v);
    const policyMin = Math.round(salesFloor(state, v, cost) * slip);
    let reason: string | undefined;
    if (price < policyMin) {
      if (wtp >= policyMin) price = Math.min(v.askingPrice, Math.round(policyMin / 10) * 10);
      else reason = `€${(policyMin - price).toLocaleString('en-GB')} under your sales policy`;
    }
    if (!reason && price >= pol.approveAbove) reason = `over your approval limit of €${pol.approveAbove.toLocaleString('en-GB')}`;
    if (reason) {
      if (mode === 'assisted' || price >= pol.approveAbove) {
        requestApproval(state, c, v, staffId, price, price - cost, reason);
        e.xp += 2;
        return false;
      }
      bump(state, 'sales', 'refused');
      lose(state, c, 'Below your sales policy');
      e.xp += 2;
      return false;
    }
  }
  ...
  let plan = undefined;
  if (c.wantsFinance && kinds.length > 1 && (mode === 'manual' || pol?.finance !== false)) {
  ...
  if (mode !== 'manual') {
    ...
    bump(state, 'sales', 'revenue', price);
    bump(state, 'sales', 'profit', result.profit);
  }
  // Automated sales are reported together at the end of the day.
  if (mode !== 'auto') pushNotice(state, 'sale', `${e.name} sold the ${vehicleName(v)} for €${price.toLocaleString('en-GB')}${plan ? ` (${plan.kind === 'lease' ? 'lease' : 'finance'} €${plan.monthly}/month)` : ''}.`);
  return true;
}
...
export function customersHour(state: GameState): void {
  ...
  for (const c of state.customers) {
    ...
    // With sales assisted or automated, advisors step in as soon as someone waits.
    const salesOn = modeFor(state, c.locationId, 'sales') !== 'manual';
    if (salesOn && now < c.leaveHour) {
      if (staffDeal(state, c) || c.status !== 'waiting') continue;
    }
    ...
    if (now >= c.leaveHour) {
      if (!(state.settings.autoStaffDeals || salesOn) || !staffDeal(state, c)) {
...
export function customersClose(state: GameState): void {
  ...
  for (const c of state.customers) {
    if (c.status === 'waiting') {
      if (!(state.settings.autoStaffDeals || modeFor(state, c.locationId, 'sales') !== 'manual') || !staffDeal(state, c)) {
