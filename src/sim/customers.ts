...
import { CLOSE_HOUR, OPEN_HOUR, absHour, boostMult, funnelStep, hasTech, locationById, nextId, pushNotice, upgradeLevel, vehicleName } from './state';
...
export function expectedLeads(state: GameState, loc: Location): { walkIn: number; online: number; total: number } {
  ...
  const boost = boostMult(state, 'leads', loc.id);
...
function runStaffDeal(state: GameState, c: Customer, v: Vehicle, staffId: string, handled: Record<string, number>): boolean {
  ...
  const team = synergyActive(state, c.locationId, 'salesteam') ? 1.1 : 1;
...
export function testDrive(state: GameState, customerId: string, opts: TestDriveOptions = {}): { ok: boolean; message: string; delta: number } {
  ...
  let delta = (v.condition - 55) / 260 + (v.presentation - 60) / 700 + gameRng.range(0.02, 0.1) + (companion ? effectiveSkill(companion) / 2500 : 0);
  if (long) delta *= 1.5;
  ...
  if (hidden.length && gameRng.chance(long ? 0.5 : 0.3)) {
  ...
  if (gameRng.chance(long ? 0.03 : 0.008)) {
  ...
  return { ok: true, message: `${c.name} took the ${vehicleName(forSale)} for a ${long ? 'long drive' : 'spin'}${companion ? ` with ${companion.name.split(' ')[0]}` : ''} and ${mood}.${note}`, delta: up };
}
