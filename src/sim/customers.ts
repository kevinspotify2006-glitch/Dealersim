...
export function expectedLeads(state: GameState, loc: Location): { walkIn: number; online: number; total: number } {
  ...
  const walkIn = focus.footfall * 1.5 * city.demand * rep * stock * showroom * seasonMult * mods.demand * pressure * (1 + campaign) * layout;
  let online = 0;
...
export function computeWtp(state: GameState, c: Customer, v: Vehicle, interest: number): number {
  ...
  if (loc) {
    // Where the car stands matters: a lit display (or the turntable) sells the dream.
    const spot = v.slotId ? loc.lot.objects.find((o) => o.id === v.slotId) : undefined;
    if (spot?.defId === 'turntable') f += 0.03;
    else if (spot && OBJ_BY_ID[spot.defId]?.slot === 'display') f += 0.015;
...
export function createCustomer(state: GameState, loc: Location, arrivalHour: number, channel: string, campaignId?: string): Customer {
  ...
  const brandPref = gameRng.chance(0.25) ? gameRng.pick(['volter', 'nordica', 'raven', 'aurelia', 'vektor', 'mistral', 'kron', 'falconer', 'monarch', 'ardent', 'sakura', 'helix']) : undefined;
  const c: Customer = {
    id: nextId(state, 'c'),
    name: customerName(),
    ...
    campaignId,
    arrivalHour,
    leaveHour: arrivalHour + gameRng.int(2, 4) + (lounge >= 2 ? 1 : 0) + reception - (ls.flow < 0.4 ? 1 : 0),
    satisfactionBonus: lounge * 2 + Math.round((ls.flow - 0.6) * 8) + reception * 2,
    status: 'scheduled',
  };
  // Trade-in
...
function arrive(state: GameState, c: Customer): void {
  ...
  const scored = listed
    ...
    .map((v) => ({ v, s: matchScore(state, c, v) }))
    .filter((x) => x.s > 0.3)
    .sort((a, b) => b.s - a.s);
  if (scored.length === 0) {
    c.status = 'left';
...
function runStaffDeal(state: GameState, c: Customer, v: Vehicle, staffId: string, handled: Record<string, number>): boolean {
  ...
  const loc = locationById(state, v.locationId);
  const finance = loc ? upgradeLevel(loc, 'finance') > 0 && gameRng.chance(0.3) : false;
