...
export function canPlace(state: GameState, loc: Location, defId: string, x: number, y: number, rot: 0 | 1, ignoreId?: string, free = false): PlaceCheck {
  ...
  for (let ty = y; ty < y + h; ty += 1) {
    for (let tx = x; tx < x + w; tx += 1) {
      ...
      if (!def.zones.includes(z)) return { ok: false, reason: `${def.name} belongs in: ${def.zones.length > 4 ? `${def.zones.slice(0, 3).map((c) => ZONE_BY_CODE[c].name).join(', ')} and ${def.zones.length - 3} more` : def.zones.map((c) => ZONE_BY_CODE[c].name).join(', ')}.`, cost };
      if (zone === null) zone = z;
