...
import { FLOOR_STYLES, LAND_TIERS, LIGHT_STYLES, OBJ_BY_ID, SYNERGIES, WALL_STYLES, ZONE_BY_CODE } from '../data/lot';
...
export function buyLand(state: GameState, loc: Location, discount = 0): { ok: boolean; message: string } {
  ...
  const city = CITY_BY_ID[loc.cityId];
  const cost = Math.round(next.cost * (city?.rent ?? 1500) / 1500 * (1 - discount));
