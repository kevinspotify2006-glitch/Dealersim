...
import { fleetFree, fleetSlot } from './systems/autocore';
import { THIRD_PARTY_MOVE } from '../data/automation';
...
export function setPrice(state: GameState, id: string, asking: number, floor?: number): { ok: boolean; message: string } {
  ...
  v.askingPrice = a;
  // Priced by hand: inventory automation leaves this car's price alone.
  v.manualPrice = true;
...
export function transferVehicle(state: GameState, id: string, toLocationId: string): { ok: boolean; message: string } {
  ...
  // Your own fleet moves it cheaper when it has room today; otherwise a carrier does.
  const own = fleetFree(state) > 0;
  const cost = own ? fleetSlot(state)! : THIRD_PARTY_MOVE;
  if (state.cash < cost) return { ok: false, message: `Transfer costs €${cost}.` };
  record(state, own ? 'Logistics' : 'Transport', -cost, `Transfer ${vehicleName(v)} to ${to.name}${own ? ' (own fleet)' : ''}`, v.locationId);
  ...
  return { ok: true, message: `${vehicleName(v)} is on ${own ? 'your own truck' : 'a truck'} to ${to.name} (arrives tomorrow, €${cost}).` };
}
