...
import { hrDaily, hrMonthly } from './systems/hr';
import { headhunterDaily } from './systems/headhunter';
import { logisticsDaily, logisticsMonthly } from './systems/logistics';
import { automationDaily, automationMonthly, automationWeekly } from './systems/automation';
...
export class Engine {
  ...
  private endDay(): void {
    ...
    // v7.3: headhunter results, HR managers and the automation rules.
    headhunterDaily(state);
    hrDaily(state);
    automationDaily(state);
    ...
    if ((state.day - 1) % 7 === 0) {
      ...
      automationWeekly(state);
    }
    state.boosts = state.boosts.filter((b) => b.until >= state.day);
    // Your own fleet collects and moves cars before the morning arrivals.
    logisticsDaily(state);
    arrivalsDaily(state);
    eventsDaily(state);
  ...
  private endMonth(): void {
    ...
    fleetMonthly(state);
    hrMonthly(state);
    logisticsMonthly(state);
    automationMonthly(state);
