...
import { effectiveSkill } from '../staff';
...
export function serviceCapacity(state: GameState, loc: Location): { lifts: number; bays: number; hours: number; bayHours: number; staff: number; speed: number } {
  ...
  const staffHours = techs.reduce((s, e) => s + 8 * (0.6 + effectiveSkill(e) / 180) * (e.role === 'technician' ? 1.15 : 1), 0);
...
function workJobs(state: GameState, loc: Location): void {
  const cap = serviceCapacity(state, loc);
  let hours = cap.hours;
  let bayHours = cap.bayHours;
  const jobs = state.serviceJobs.filter((j) => j.locationId === loc.id && j.status !== 'done').sort((a, b) => a.dueDay - b.dueDay);
  const evOk = canServiceEv(state, loc);
  const advisor = staffAt(state, loc.id).find((e) => e.role === 'advisor' && e.trainingDaysLeft <= 0);
  for (const j of jobs) {
    const def = SERVICE_BY_ID[j.type];
    if (def.ev && !evOk) {
      j.status = 'done';
      j.labour = 0;
      continue;
    ...
    if (!j.partsReserved) {
      if (!takeParts(loc, j.parts)) {
        j.status = 'parts';
        continue;
      }
      j.partsReserved = true;
    }
    const pool = def.bay ? bayHours : hours;
    if (pool <= 0) {
      j.status = 'waiting';
      continue;
    }
    j.status = 'working';
    const used = Math.min(pool, j.hours);
    j.hours -= used;
    if (def.bay) bayHours -= used; else hours -= used;
    if (j.hours <= 0.01) finishJob(state, loc, j, advisor ? effectiveSkill(advisor) : 0);
  }
}

function finishJob(state: GameState, loc: Location, j: ServiceJob, advisorSkill: number): void {
  j.status = 'done';
  ...
  const happy = !late && gameRng.chance(0.75 + loc.reputation / 400);
