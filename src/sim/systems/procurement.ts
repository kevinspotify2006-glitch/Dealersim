...
function searchDay(state: GameState, e: Employee, m: BuyMandate): void {
  ...
  for (const f of take) {
    ...
    if (m.autoApprove && p.estMargin >= m.minMargin * 1.3 && p.risk !== 'high' && (!m.autoLimit || p.price <= m.autoLimit)) {
...
export function approveProposal(state: GameState, id: string, auto = false): { ok: boolean; message: string } {
  ...
  if (state.automation) {
    state.automation.spent.procurement += p.price;
    bump(state, 'procurement', auto ? 'autoBought' : 'bought');
    bump(state, 'procurement', 'spent', p.price);
  }
  if (e) {
  ...
  if (auto) pushNotice(state, 'good', `🚗 ${e?.name.split(' ')[0] ?? 'Your buyer'} bought ${vehicleName(v)} for ${fmt(p.price)} (expected margin ${fmt(p.estMargin)}). It is on its way.`, undefined, 'auto');
