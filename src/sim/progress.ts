...
import { PHYSICAL_UPGRADES, capacityOf, locationById, nextId, occupying, pushNotice, staffAt } from './state';
...
export function checkAchievements(state: GameState): void {
  ...
  if (state.stats.sold >= 1) unlock(state, 'firstsale');
  if (state.stats.sold >= 10) unlock(state, 'tensales');
  if (state.stats.sold >= 100) unlock(state, 'sold100');
  if (value >= 100000) unlock(state, 'value100k');
  if (value >= 1000000) unlock(state, 'value1m');
  if (state.locations.length >= 2) unlock(state, 'second');
  if (state.stats.customersTotal >= 1000) unlock(state, 'customers1000');
  if (state.stats.perfectReviews >= 1) unlock(state, 'perfect');
  if (state.stats.biggestDeal >= 100000) unlock(state, 'bigdeal');
  if (state.stats.bestMonthProfit >= 50000) unlock(state, 'bestmonth');
  if (state.companyLevel >= 7) unlock(state, 'empire');
  if (state.employees.length >= 5) unlock(state, 'hire5');
  if (state.reputation >= 90) unlock(state, 'rep90');
  if ((state.stats.evSold ?? 0) >= 25) unlock(state, 'evspecial');
  if ((state.stats.loansRepaid ?? 0) >= 1) unlock(state, 'debtfree');
  if ((state.stats.rareBought ?? 0) >= 1) unlock(state, 'rarefind');
