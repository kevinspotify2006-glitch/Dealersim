...
export function modelProfile(state: GameState, modelId: string): ModelProfile | undefined {
  ...
  const p: ModelProfile = {
    id: m.id, brandId: m.brandId, brand: brand.name, model: m.name, body: m.body, category: m.category,
    type: modelType(m), segment: modelSegment(m), fuels: m.fuels, purchasePrice, salePrice,
    margin: salePrice - purchasePrice - estimatedCosts(salePrice),
    demand, demandLabel: demand >= 62 ? 'Very high' : demand >= 45 ? 'High' : demand >= 28 ? 'Medium' : 'Low',
    ...
    typicalYear: ex.year, typicalMileage: ex.mileage, maxHp, available: m.yearFrom <= yearOf(state.day),
  };
