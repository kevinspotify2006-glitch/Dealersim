...
import { BRAND_BY_ID, COLORS, MODELS, OPTIONS } from '../data/vehicles';
import { ISSUE_TEMPLATES, PREP_BY_ID, SOURCE_BY_ID, TRADEIN_SOURCE, SELLER_NAMES, STRATEGY_BY_ID } from '../data/game';
import type { PrepActionDef, SourceDef } from '../data/game';
import { gameRng } from './rng';
import { clamp } from './util';
import { yearOf } from './format';
import { activeLocation, locationById, nextId, pushNotice, staffAt, upgradeLevel, vehicleName } from './state';
import { assignSlots, findFreeSlot, isVisibleSlot, lotStats, slotKindOf } from './lot';
...
function pickModel(state: GameState, opts: GenOptions) {
  ...
  if (opts.source === 'special') {
    ...
    pool = pool.filter((m) => rnd.next() > m.rarity * 0.9);
...
export function generateVehicle(state: GameState, opts: GenOptions): Vehicle {
  ...
  let year: number;
  if (model.category === 'Classic' || model.category === 'Rare') {
  ...
  const perYear = model.category === 'Van' ? 22000 : model.category === 'Classic' || model.category === 'Rare' ? 3500 : model.category === 'Performance' || model.category === 'Luxury' ? 9000 : 15000;
  const mileage = Math.max(900, Math.round((Math.max(0.4, age) * perYear * rnd.range(0.45, 1.6)) / 100) * 100);
  const color = rnd.pick(COLORS);
  const optionCount = clamp(Math.round(rnd.range(0, 3) + model.trims.length * 0.3 + (model.basePrice > 45000 ? 3 : 0)), 0, 8);
  const options = rnd.sample(OPTIONS, optionCount).map((o) => o.name);
  const quality = opts.quality ?? 0;
  const condition = clamp(Math.round(92 - age * 2.1 - mileage / 9000 + rnd.around(0, 14) + quality * 10), 18, 99);
  const overstatement = Math.max(0, rnd.around(source.conditionSpread * 0.55, source.conditionSpread * 0.6));
  const apparentCondition = clamp(Math.round(condition + overstatement), 20, 99);
  const issues = makeIssues(state, source, age, mileage, brand.reliability);
  ...
  const history: string[] = [
    ...
    rnd.chance(0.65) ? 'Service book present' : 'Partial service history',
  ];
  if (rnd.chance(0.2)) history.push('Imported');
  if (rnd.chance(0.12)) history.push('Ex-fleet vehicle');
  ...
  const v: Vehicle = {
    ...
    condition,
    apparentCondition,
    presentation: clamp(Math.round(rnd.range(20, 55) + quality * 10), 5, 80),
    issues,
    inspectionLevel: 0,
    photosPro: false,
    reliability: brand.reliability,
    popularity: clamp(brand.popularity * 0.7 + rnd.range(0, 0.3), 0, 1),
    rarity: model.rarity,
    source: opts.source,
    seller: opts.source === 'private' ? 'Private seller' : opts.source === 'tradein' ? 'Customer trade-in' : rnd.pick(SELLER_NAMES.filter((s) => s !== 'Private seller')),
    history,
    purchasePrice: 0,
    offerPrice: 0,
...
export function createOffer(state: GameState, sourceId: SourceId, locationId: string): Vehicle {
  const rnd = gameRng;
  ...
  // Buyers find more of what the dealership specialises in.
  const focus = STRATEGY_BY_ID[state.locations.find((l) => l.id === locationId)?.strategy ?? 'balanced'];
  const categories = focus?.categories && rnd.chance(0.55) ? focus.categories : undefined;
  let v = generateVehicle(state, { source: sourceId, locationId, categories });
  for (let i = 0; i < 6 && sourceId !== 'special' && bookValue(state, v, mods) * TRADE_FACTOR > cap; i += 1) {
    v = generateVehicle(state, { source: sourceId, locationId, maxBase: cap * 1.6 });
  }
  ...
  const factor = rnd.range(source.priceFactor[0], source.priceFactor[1]) * mods.purchase * buyingPower(state) * repTerms;
  v.offerPrice = roundPrice(apparent * factor);
  v.expiresDay = state.day + rnd.int(2, 5);
  if (sourceId === 'auction' || sourceId === 'special') {
...
export function detectionChance(state: GameState, locationId: string, advanced: boolean): number {
  ...
  return clamp(base + best / 250 + equipment * 0.15 + eye, 0.1, 0.97);
}
...
export function prepCost(state: GameState, v: Vehicle, actionId: string): number {
  ...
  if (actionId === 'photos') {
    const location = locationById(state, v.locationId) ?? activeLocation(state);
    if (upgradeLevel(location, 'detailing') >= 3 || (lotStats(location.lot).effects.photo ?? 0) > 0) return 0;
    return action.cost;
  }
  ...
  return Math.round((cost * mult * discount) / 10) * 10;
}
  ...
  for (const v of state.vehicles) {
    ...
    if (task.daysLeft <= 0) {
      ...
      if (v.prep.length === 0) {
        ...
        const hasManager = staffAt(state, v.locationId).some((e) => e.role === 'manager');
        if (auto && hasManager) {
...
export function appealScore(state: GameState, v: Vehicle): number {
  ...
  const spot = location && slotKindOf(location, v.slotId) === 'display' ? 0.05 + showroom * 0.02 : showroom * 0.01;
...
export function listingQuality(state: GameState, v: Vehicle): number {
  ...
  return clamp(
    0.28 * (v.presentation / 100) + (v.photosPro ? 0.22 : 0.04) + 0.12 * (marketing / 3) + 0.2 * priceComp + 0.1 * (state.reputation / 100) + mktStaff / 1000,
    0.05,
    1,
  );
}
...
export function presentationDecay(state: GameState): void {
  for (const v of state.vehicles) {
    ...
    const indoor = location && slotKindOf(location, v.slotId) === 'display';
    const showroom = location ? upgradeLevel(location, 'showroom') : 0;
    const decay = indoor ? 0.35 - showroom * 0.03 : 1.2;
    v.presentation = clamp(v.presentation - Math.max(0.2, decay), 0, 100);
  }
}
