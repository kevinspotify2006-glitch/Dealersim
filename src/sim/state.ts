...
export function pushNotice(state: GameState, kind: Notice['kind'], text: string): Notice {
  state.noticeCounter += 1;
  const notice: Notice = { id: state.noticeCounter, day: state.day, kind, text, read: false };
...
export function emptyKpi(): GameState['kpi'] {
  return { waitHours: 0, waitCount: 0, financeDeals: 0, financeOffers: 0, productsSold: 0, serviceRevenue: 0, partsRevenue: 0, financeIncome: 0, deliveries: 0 };
}
