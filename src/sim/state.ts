...
export function pushNotice(state: GameState, kind: Notice['kind'], text: string, action?: Notice['action'], tag?: Notice['tag']): Notice {
  state.noticeCounter += 1;
  const notice: Notice = { id: state.noticeCounter, day: state.day, kind, text, read: false, ...(action ? { action } : {}), ...(tag ? { tag } : {}) };
