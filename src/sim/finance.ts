...
export const INCOME_CATEGORIES: TxCategory[] = ['Vehicle sale', 'Extras'];
...
export function record(state: GameState, category: TxCategory, amount: number, description: string, locationId?: string): Transaction {
  ...
  } else if (safe < 0 && category !== 'Vehicle purchase' && category !== 'Loan' && category !== 'Expansion' && category !== 'Upgrades' && category !== 'Construction'
    && category !== 'Transport' && category !== 'Repairs' && category !== 'Detailing' && category !== 'Inspection') {
...
export function companyValue(state: GameState): number {
  ...
  return Math.round(state.cash + inventoryValue(state) * 0.92 + propertyValue(state) - debt(state) + goodwill);
}
...
export function loanRate(state: GameState, product: LoanProduct): number {
  ...
  return clamp(product.rate + repAdj - accountantDiscount(state) * 0.02, 0.03, 0.2);
}
...
/** Monthly: rent, salaries, insurance, loan payments, tax. */
export function monthlyCosts(state: GameState): void {
  // Salaries
  const payroll = sum(state.employees, (e) => e.salary);
  if (payroll > 0) record(state, 'Salaries', -payroll, `Payroll: ${state.employees.length} staff`);
  for (const loc of state.locations) {
  ...
  const security = state.locations.reduce((m, l) => m + Math.min(6, lotStats(l.lot).effects.security ?? 0), 0) / Math.max(1, state.locations.length);
  ...
  if (taxable > 0) {
    const rate = 0.21 * (1 - accountantDiscount(state));
...
export function dailyCosts(state: GameState): void {
  for (const loc of state.locations) {
    ...
    const utilities = Math.round((18 + ls.power + ls.indoorTiles * 0.12 * lighting + capacityOf(loc) * 0.3) * (loc.lot.open ? 1 : 0.6));
