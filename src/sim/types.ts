...
export interface Vehicle {
  ...
  manualPrice?: boolean;        // priced by hand: automation leaves the price alone
  pickedUp?: boolean;           // v7.3: collected by your own fleet
  repricedDay?: number;         // last automatic reprice
...
export type Role = 'sales' | 'mechanic' | 'detailer' | 'buyer' | 'manager' | 'accountant' | 'marketing'
  ...
  | 'photographer' | 'prep' | 'procurement' | 'admin' | 'hr' | 'logistics';
...
export interface Employee {
  ...
  // v7.3 HR
  /** Years of experience in the trade (candidates from a headhunter state it). */
  experience?: number;
  /** The HR manager looking after this employee. */
  hrBy?: string;
  /** HR managers: run their people automatically (training, promotions, pay, cover). */
  hrAuto?: boolean;
  /** Headhunter candidates: traits were checked (level 3+ headhunters). */
  traitsKnown?: boolean;
  /** Headhunter candidates: where they come from ("Rival: Autohaus…"). */
  source?: string;
}
...
export type PricingPolicy = 'aggressive' | 'market' | 'premium' | 'conservative' | 'maxprofit' | 'fast';
...
export type TxCategory =
  ...
  | 'Finance income' | 'Service' | 'Parts' | 'Parts purchase' | 'Commission' | 'Bonus' | 'Research' | 'Dealer bonus' | 'Acquisition' | 'Delivery' | 'Recruitment' | 'Logistics' | 'Automation';
...
export interface Notice {
  ...
  action?: NoticeAction;
  /** Automation messages (filterable in the notification centre). */
  tag?: 'auto';
}
...
export interface BuyMandate {
  ...
  /** Set by procurement automation: auto-buy only up to this price. */
  autoLimit?: number;
...
// ---------------------------------------------------------------- management & automation (v7.3)
export type AutoMode = 'manual' | 'assisted' | 'auto';
export type Dept = 'procurement' | 'inventory' | 'sales' | 'service' | 'parts' | 'hr' | 'logistics' | 'marketing';
export type AutoPriority = 'low' | 'normal' | 'high' | 'critical';

export interface ProcurementPolicy { stockMin: number; stockTarget: number; maxPrice: number; minMargin: number; risk: RiskAppetite; categories: Category[]; fuels: FuelType[]; kmMax: number; autoBuyUnder: number; monthlyBudget: number }
export interface InventoryPolicy { minStock: number; targetStock: number; maxStock: number; repriceDays: number; clearDays: number; pricing: PricingPolicy; autoList: boolean }
export interface SalesPolicy { minMargin: number; maxDiscount: number; finance: boolean; tradeIns: boolean; approveAbove: number }
export interface HrPolicy { targets: Partial<Record<Role, number>>; autoReplace: boolean; trainingBudget: number; trainBelow: number; promoteSkill: number; promotePerf: number; promoteMonths: number; salaryReview: boolean }
export interface ServicePolicy { autoFix: boolean; vipFirst: boolean }
export interface PartsPolicy { minFactor: number; targetFactor: number; express: boolean }
export interface MarketingPolicy { monthlyBudget: number; minRoi: number; channels: string[] }
export interface LogisticsPolicy { balanceStock: boolean; pickUp: boolean }

export interface AutomationPolicies {
  modes: Record<Dept, AutoMode>;
...
/** Per-location overrides of the HQ (group) policy. */
export interface LocalPolicy {
  modes?: Partial<Record<Dept, AutoMode>>;
  stockMin?: number;
  stockTarget?: number;
  stockMax?: number;
...
export interface AutoException {
  id: string;
  day: number;
  dept: Dept;
...
export interface AutoReport {
  day: number;
  /** Per department: counters ("bought", "spent", "sold"…). */
  lines: Partial<Record<Dept, Record<string, number>>>;
}

export type RuleMetric = 'stock' | 'cash' | 'aged' | 'partsLow' | 'waiting' | 'staff';
export type RuleAction = 'reprice' | 'startBuying' | 'pauseBuying' | 'orderParts' | 'campaign' | 'recruit' | 'notify';
export interface CustomRule {
  id: string;
  name: string;
  enabled: boolean;
  metric: RuleMetric;
  /** For "staff": the role; for "aged": the days. */
  arg?: string;
  op: '<' | '>';
  value: number;
  action: RuleAction;
  /** Action parameter: percentage, channel id, role… */
  param?: string;
  priority: AutoPriority;
  cooldown: number;             // days between firings
  lastDay?: number;
  fired?: number;
}
...
export interface AutomationState {
  ...
  local: Record<string, LocalPolicy>;
  exceptions: AutoException[];
  approvals: SaleApproval[];
  reports: AutoReport[];
  rules: CustomRule[];
  /** Spent this month by automated budgets. */
  spent: { training: number; marketing: number; procurement: number };
  /** Employees who left, for automatic replacement. */
  left: { role: Role; locationId: string; day: number }[];
}

export interface HeadCriteria {
  minSkill: number;
  maxSalary: number;
  minExperience: number;
  traits: Partial<Record<SkillId, number>>;
  contract: ContractKind | 'any';
  ageMin: number;
  ageMax: number;
  specialization?: string;
}

export interface HeadSearch {
  id: string;
  role: Role;
  locationId: string;
  criteria: HeadCriteria;
  status: 'searching' | 'ready' | 'closed';
  startDay: number;
  readyDay: number;
  expires: number;
  candidates: Employee[];
...
export interface HeadhunterState {
  level: number;                // 0 = none hired
  searches: HeadSearch[];
  placed: number;
  since?: number;
}

export type LogiAssetType = 'van' | 'truck' | 'carrier' | 'hub';
export interface LogiAsset { id: string; type: LogiAssetType; locationId: string; boughtDay: number; condition: number; downUntil?: number }
export interface LogisticsState {
  fleet: LogiAsset[];
  movedMonth: number;
  costMonth: number;
  /** Fleet moves used today (reset every morning). */
  usedDay?: number;
  used?: number;
}
