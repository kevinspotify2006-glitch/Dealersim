...
(async () => {
  ...
  const click = async (sel) => { const l = page.locator(sel).first(); await l.scrollIntoViewIfNeeded(); if (PHONE) await l.tap(); else await l.click(); await wait(); };
  ...
  check(['dealership', 'inventory', 'people', 'service', 'business', 'settings'].every((x) => sections.includes(x)) && sections.length === 6, `six sections in the navigation (${sections.join(',')})`);
  const chips = await G(() => [...document.querySelectorAll(window.innerWidth <= 900 ? '.status-strip .sb-chip' : '.hud.statusbar .sb-chip')].filter((c) => c.getBoundingClientRect().width > 0).map((c) => c.dataset.hud));
  check(chips.includes('cash') && chips.includes('stock') && chips.includes('staff') && chips.includes('service'), `status bar shows ${chips.join(', ')}`);
  await click(`${PHONE ? '.status-strip' : '.hud.statusbar'} .sb-chip[data-hud=stock]`);
