...
import { buyLand, canPlace, carDropCheck, carSize, duplicateSpot, footprint, lineCheck, lotStats, moveObject, nextPlot, paintZone, placeLine, placeObject, placeVehicle, zoneAt, zoneCost } from '../../sim/lot';
...
export function worldView(ctx: Ctx): View {
  ...
  const paintBuild = (): void => {
    ...
    paletteHost.replaceChildren(buildPalette({
      ...
      loc: () => loc,
      tool: () => ui.tool,
      setTool: (t) => { ui.tool = t; ui.selected = null; bullHover = null; pickReturn = false; closePop(); paintBuild(); if (isPhone() && t.kind !== 'select' && t.kind !== 'bulldoze' && t.kind !== 'pick' && t.kind !== 'area') collapsePalette(true); },
  ...
  const paintConfirm = (): void => {
    ...
    if (t.kind === 'place' || t.kind === 'move') {
      ...
      const sg = stampGhost();
      const tpl = ROOM_TEMPLATE_BY_ID[t.templateId];
      label = sg ? sg.label : `${tpl.icon} ${tpl.name} — ${phone ? 'tap where it should go' : 'click to place · R rotate'}`;
      ok = !!sg && sg.ok;
      doIt = sg && (phone || t.x !== undefined) ? commitStamp : null;
      doLabel = 'Place';
    } else if (t.kind === 'bulldoze') {
      const b = bulldozePreview();
      label = b?.label ? `💥 ${b.label} — click to bulldoze` : `💥 Bulldoze — ${phone ? 'tap' : 'click'} an object (Delete also works)`;
      ok = true;
    } else if (t.kind === 'pick') {
      label = `✋ Move — ${phone ? 'tap' : 'click'} anything built to pick it up`;
    } else if (t.kind === 'area') {
      const ap = areaPreview();
      label = ap ? ap.label : `⬚ Bulldoze area — drag a rectangle over the map`;
      ok = !!ap && ap.ok;
      const r = dragRect();
      ...
      label = rp ? rp.label : `🏗️ Create room — drag a rectangle where the room should be`;
      ok = !!rp && rp.ok;
    } else {
    ...
    const key = `${label}|${ok}|${!!doIt}|${doLabel}|${roomRect ? 'r' : ''}`;
    if (key === confirmKey) return;
    confirmKey = key;
    confirmBar.replaceChildren(h('span', { class: 'wc-label', text: label }));
    if (t.kind === 'place' || t.kind === 'move' || t.kind === 'car' || t.kind === 'stamp') confirmBar.appendChild(h('button', { class: 'btn small', title: 'Rotate (R)', data: { cb: 'rotate' }, on: { click: rotateTool } }, icon('rotate', 14), phone ? '' : 'Rotate'));
    if (roomRect) {
