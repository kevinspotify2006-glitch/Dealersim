...
(async () => {
  ...
  const press = async (sel) => { if (PHONE && sel.startsWith('.bp-')) { await page.evaluate(() => document.querySelector('.world-pop.open .wm-close')?.click()); await page.waitForTimeout(60); } return PHONE ? page.tap(sel) : page.click(sel); };
  ...
  await press('.bp-chip[data-filter=cheap]');
  ...
  await press('.bp-chip[data-filter=all]');
  ...
  await press('.bp-chip[data-filter=unlocked]');
  ...
  for (const cat of ['structure', 'showroom', 'vehicles', 'office', 'customer', 'service', 'decoration', 'outdoor', 'security']) {
  ...
  await press('.bp-tab[data-cat=customer]');
  ...
  const dupSel = await G(() => window.__CDMT_WORLD__.ui().selected);
  if (!dupSel || dupSel.id === plantId) fail('duplicate did not select the copy');
  n0 = await objs();
  if (PHONE) await press('.world-pop.open .btn:has-text("Copy")'); else { await tapAt(await G((id) => window.__CDMT_WORLD__.objScreen(id), plantId)); await key('Control+c'); }
  if (!PHONE) {
    await key('Control+v');
  ...
  await tapAt(await G((id) => window.__CDMT_WORLD__.objScreen(id), plantId));
  n0 = await objs();
  if (PHONE) await press('.world-pop.open .btn:has-text("Delete")'); else await key('Delete');
  ...
  if (dearId && !PHONE) {
    ...
    if (sel && sel.kind === 'object') {
      ...
      await key('Delete');
      const dialog = await page.locator('.modal-overlay .confirm-text').count();
      if (dialog) { await page.click('.modal-overlay >> text=Cancel'); await wait(); if (!(await obj(id))) fail('cancelled delete still removed the object'); } else log('selected object is cheap, no confirm:', cost && cost.defId);
    }
  ...
  await press('.bp-tool[data-bt=grid]');
  ...
  await press('.bp-tool[data-bt=snap]');
