...
(async () => {
  ...
  const click = async (sel) => { const l = page.locator(sel).first(); await l.scrollIntoViewIfNeeded().catch(() => {}); if (PHONE) await l.tap(); else await l.click(); await wait(); };
  ...
  check(['select', 'pick', 'bulldoze', 'area'].every((m) => modes.includes(m)), `build modes: ${modes.join(', ')}`);
  ...
  // ---- area bulldoze
  await press('.bp-mode[data-bt=area]');
