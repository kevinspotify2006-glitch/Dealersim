...
(async () => {
  ...
  await G(() => { const G = window.__CDMT__; const s = G.state; s.cash += 50000; const r = G.lot.buildQuickRoom(s, s.locations[0], 'workshop'); if (!r.ok) throw new Error(r.message); G.app.go('staff'); });
  await page.waitForTimeout(100);
