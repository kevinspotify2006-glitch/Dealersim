...
(async () => {
  ...
  await G(() => { const G = window.__CDMT__; const s = G.state; s.cash += 50000; const r = G.lot.buildQuickRoom(s, s.locations[0], 'workshop'); if (!r.ok) throw new Error(r.message); G.app.go('company', { tab: 'staff' }); });
  await page.waitForTimeout(100);
  ...
  await page.locator('tr.row-clickable').last().click();
  await page.waitForSelector('.modal-card');
  await page.click('.modal-footer >> text=Train');
  await page.waitForTimeout(100);
