...
(async () => {
  ...
  await press('.world-pop >> text=Inspect');
  ...
  for (let i = 0; i < 150 && !served; i += 1) {
    ...
    if (!(await page.locator('.world-pop.open').count())) { await page.evaluate((id) => window.__CDMT__.app.serve(id), w); } else {
      ...
      await press('.world-pop >> text=Talk');
      await page.waitForTimeout(80);
      const td = page.locator('.world-pop button:has-text("Test drive")');
      if (await td.isEnabled()) await td.click();
      await page.waitForTimeout(100);
      ...
      await press('.world-pop >> text=Negotiate');
    }
  ...
  await page.evaluate(() => { const s = window.__CDMT__.state; s.companyLevel = 3; s.cash += 200000; window.__CDMT__.app.go('company', { tab: 'locations' }); });
