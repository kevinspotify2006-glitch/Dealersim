...
const ROUTES = ['dealership', 'inventory', 'people', 'service', 'business', 'management', 'settings'];
const TABS = {
  inventory: ['stock', 'buy', 'buyers', 'ageing', 'fleet'],
  people: ['staff', 'hire', 'planning', 'customers', 'visitors', 'hr', 'headhunter'],
  management: ['overview', 'automation', 'rules', 'reports', 'managers', 'hr', 'headhunter', 'procurement', 'inventory', 'sales', 'service', 'marketing', 'logistics', 'finance'],
...
(async () => {
  ...
  for (const v of VIEWS) {
    ...
    for (const r of ROUTES) {
      ...
      if (phone && r === 'management') {
        // Phones: Management sits behind the sixth tab, More (two taps).
        await page.click('.mobile-nav .nav-bar .nav-item[data-section="more"]');
        await page.waitForTimeout(150);
        await page.click('.more-sheet .more-item[data-more="management"]');
        await page.waitForTimeout(150);
        if ((await page.evaluate(() => window.__CDMT__.app.route)) !== 'management') fail(`${name}: management not reached from More`);
        continue;
      }
