...
(async () => {
  ...
  // ---- six-item navigation (the sixth is More); Settings and the money behind the header
  const navIds = await G(() => [...document.querySelectorAll('.mobile-nav .nav-bar .nav-item')].map((b) => b.dataset.section));
  check(navIds.length === 6 && navIds[5] === 'more' && !navIds.includes('settings'), `bottom nav: ${navIds.join(', ')}`);
  await tap('#m-hud');
