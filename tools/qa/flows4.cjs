...
(async () => {
  ...
  const click = async (sel) => { await page.locator(sel).first().scrollIntoViewIfNeeded(); if (PHONE) await page.locator(sel).first().tap(); else await page.locator(sel).first().click(); await wait(); };
  ...
  await wait(120);
  if (PHONE) await click('.staff-card >> nth=-1'); else await click('tr.row-clickable >> nth=-1');
